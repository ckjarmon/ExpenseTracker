...
extern "C" JNIEXPORT jstring JNICALL Java_com_kyeou_expensetracker_AddExpense_addTrans (JNIEnv* env, jobject, jstring name, int day, int month, int year, float amount, jstring TRANS_JSON_PARM) {
    t = new TRANS_HANDLE(env->GetStringUTFChars(name, nullptr), new Date(month, day, year), amount);
   return env->NewStringUTF(t->addTrans(env->GetStringUTFChars(TRANS_JSON_PARM, nullptr)).c_str());
...
extern "C" JNIEXPORT jstring JNICALL Java_com_kyeou_expensetracker_loginPage_userManage(JNIEnv * env, jobject, jstring USER_INFO_JSON, jstring TRANS_INFO_JS) {
    u = new USER_HANDLE(env->GetStringUTFChars(_USER, nullptr));
    return env->NewStringUTF(u->USERDUMP().c_str());
}