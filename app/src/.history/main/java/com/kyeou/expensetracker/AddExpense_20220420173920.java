...
public class AddExpense extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
 ...
 if (temp_parm.equals("")) {new WriteReadHandle().WriteHandle("transactions.json", "[]");}
        ...
        new WriteReadHandle().WriteHandle("transactionsJSON.json", addTrans(description, day, month, year, Float.valueOf(amount).floatValue(),
                ReadHandle("transactionsJSON.json")));
        recordDebits();
        new WriteReadHandle().WriteHandle("user.json", getUSERSJSON());
        new WriteReadHandle().WriteHandle("transactionsJSON.json", getTRANSJSON());
