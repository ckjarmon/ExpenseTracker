...
namespace GLOBAL_VARS
{
    ...
    {
        int red = 1;
        float max = std::numeric_limits<float>::ma();
        std::string ttp = "";
        int ttpw = 0;
        while (red != 8)
        {
            ...
            for (json::iterator it = TRANSACTIONS_JSON.begin(); it != TRANSACTIONS_JSON.end(); ++it)
            {
                if ((*it)["Rank"] == INT_MAX && (*it)["Amount: "] > max)
                {
                    // ttp = (*it)["TRANS_STRING"];
                    ttpw = t_count;
                    max = TRANSACTIONS_JSON[ttpw]["Amount: "];
                    std::cout << "NEW MAX __>> "<< max << std::endl;
                }
                t_count++;
                 std::cout << t_count << std::endl;
            } // end for
            TRANSACTIONS_JSON[ttpw]["Ranks"] = red;
