...
namespace GLOBAL_VARS
{
    ...
    void establishRanks()
    {
        ...
        float curr_big, max;
        curr_big = std::numeric_limits<float>::min();
        ...
        while (rank_couter != 2)
        {
            ...
            for (json::iterator it = TRANSACTIONS_JSON.begin(); it != TRANSACTIONS_JSON.end(); ++it)
            {
                ...
                if ((*it)["Amount: "] > curr_big && (*it)["Amount: "] < max && (*it)["Rank"] == INT_MAX)
                {
                    curr_big = (*it)["Amount: "];
                    curr_big_don = it_count;
                }
                it_count++;
            } // end for
            max = TRANSACTIONS_JSON[curr_big_don]["Amount: "] ;
            TRANSACTIONS_JSON[curr_big_don]["Rank"] = rank_couter + 1;
            //RANKS_JSON[rank_couter] = TRANSACTIONS_JSON[curr_big_don];

RANKS_JSON[rank_couter]["Name: "] = TRANSACTIONS_JSON[curr_big_don]["Name: "];
        RANKS_JSON[rank_couter]["Date: "] = TRANSACTIONS_JSON[curr_big_don]["Date: "];
        RANKS_JSON[rank_couter]["Amount: "] = TRANSACTIONS_JSON[curr_big_don]["Amount: "];
       RANKS_JSON[rank_couter]["ATTRIBUTE->RECORDED_BOOL: "] = TRANSACTIONS_JSON[curr_big_don]["ATTRIBUTE->RECORDED_BOOL: "];
      

       RANKS_JSON[rank_couter]["TRANS_STRING"] = TRANSACTIONS_JSON[curr_big_don]["ATTRIBUTE->RECORDED_BOOL: "];
...
public:
    ...
    std::string addTrans(std::string t)
    {
        ...
        TRANSACTIONS_JSON[A_O_T]["Rank"] = INT_MAX;

        TRANSACTIONS_JSON[A_O_T]["TRANS_STRING"] = this->getTransString();
