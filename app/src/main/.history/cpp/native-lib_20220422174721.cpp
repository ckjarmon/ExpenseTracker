...
extern "C" JNIEXPORT jstring JNICALL Java_com_kyeou_expensetracker_loginPage_userLOGIN(JNIEnv *env, jobject thiz,  jstring user__info__json,   jstring trans__info__json)
{
    u = new USER_HANDLE(env->GetStringUTFChars(user__info__json, nullptr), env->GetStringUTFChars(trans__info__json, nullptr));
    return env->NewStringUTF(u->USERDUMP().c_str());
}